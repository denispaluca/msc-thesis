...
We also looked at different interpreters for \ac{TEAL} that simulate the \ac{AVM} such as teal-interpreter \cite{} and the AlgoBuilder Runtime.
...
\begin{itemize}
    ...
    \item teal-interpreter has had no commits in more than one year, supports only up to \ac{TEAL} version 5 and does not support all opcodes.
\end{itemize}

\paragraph*{Fuzzer} The fuzzer is implemented in Python and it uses the Algorand Python SDK which connects to the REST-API of the local network.
...
Although Python is not the fastest language, we expected most of the time to be spent on the REST-API calls to the local network.
